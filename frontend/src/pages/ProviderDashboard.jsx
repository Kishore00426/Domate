...
import { AlertCircle, User, CheckCircle, Upload, Briefcase, MapPin, Trash2, X, Pencil, Edit2, FileText } from 'lucide-react';
...
const ProviderDashboard = () => {
    ...
    // const { user } = useOutletContext(); // Context might be available if Layout provides it, but we fetch manually below 
    ...
    // Mobile Navigation Items corrected for Tab System
    const mobileNavItems = [
        { id: 'profile', label: t('dashboard.profile') || 'Profile', icon: '👤' },
        { id: 'services', label: t('dashboard.myServices') || 'My Services', icon: '💼' },
        { id: 'bookings', label: t('dashboard.myBookings') || 'Bookings', icon: '📅' },
        { id: 'documents', label: t('dashboard.verificationDocuments') || 'Documents', icon: '📄' },
    ];
        ...
        <div className="space-y-8">
            ...
            <div className="bg-white p-6 md:p-8 rounded-3xl shadow-sm border border-gray-100 flex flex-col md:block relative transition-all">
                <div className="flex justify-between items-start">
                    ...
                    {/* Mobile Actions: Logout or Back */}
                    <div className="md:hidden flex gap-2">
                        {activeTab !== 'overview' && (
                            <button
                                onClick={() => setActiveTab('overview')}
                                className="p-2 bg-gray-100 text-black rounded-lg hover:bg-gray-200 transition-colors shadow-sm"
                                title="Back to Menu"
                            >
